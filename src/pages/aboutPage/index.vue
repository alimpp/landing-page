<template>
  <div class="about-page application_animation">
    <div class="row">
      <div class="col-lg-6 image-container">
        <img
          src="https://media.istockphoto.com/id/1352825038/photo/professional-male-automotive-engineer-wearing-hard-hat-walking-using-laptop-monitoring.jpg?b=1&s=170667a&w=0&k=20&c=tT2OqQMFqqNGFqqvXnR_s1GosM54uMSBFB6lUyo6q8Y="
          alt="image"
        />
      </div>
      <div class="col-lg-6">
        <coreDivider title="About Us" />
        <p>
          In publishing and graphic design, Lorem ipsum is a placeholder text
          commonly used to demonstrate the visual form of a document or a
          typeface without relying on meaningful content. Lorem ipsum may be
          used as a placeholder before final copy is available
        </p>
        <coreDivider title="Company" />
        <p>
          In publishing and graphic design, Lorem ipsum is a placeholder text
          commonly used to demonstrate the visual form of a document or a
          typeface without relying on meaningful content. Lorem ipsum may be
          used as a placeholder before final copy is available
        </p>
      </div>
    </div>
    <div class="row">
      <coreDivider title="Experience" />
      <div class="col-lg-12" v-for="user in usersExperince" :key="user.id">
        <coreTimeLine
          :username="user.username"
          :date="user.date"
          :image="user.image"
          :message="user.message"
        />
      </div>
    </div>
  </div>
</template>

<script>
import coreDivider from "@/components/core/coreDivider";
import coreTimeLine from "@/components/core/coreTimeLine";
export default {
  components: { coreDivider, coreTimeLine },
  data() {
    return {
      usersExperince: [
        {
          username: "Jhon Doe",
          date: "2022/12/2",
          image:
            "data:image/jpeg;base64,",
          message:
            "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Ducimus praesentium doloremque rerum, aspernatur vero tempore tenetur architecto nisi quos animi facere blanditiis quis sequi, cupiditate deleniti nihil ipsum debitis blanditiis",
        },
        {
          username: "Mika Daniels",
          date: "2021/10/22",
          image:
            "https://tabib.ae/uploads/profile/free-profile-photo-whatsapp-4_1617693292_1625640135.png",
          message:
            "Ducimus praesentium doloremque rerum, aspernatur vero tempore tenetur architecto nisi quos animi facere blanditiis quis sequi, cupiditate deleniti nihil ipsum debitis blanditiis",
        },
        {
          username: "Rita Makkay",
          date: "2020/9/22",
          image:
            "https://853239.smushcdn.com/1831098/wp-content/uploads/2020/12/portrait-of-a-young-african-american-business-woma-PEK9XL4.jpg?lossy=1&strip=1&webp=1",
          message:
            "Ducimus tenetur architecto nisi quos animi facere blanditiis quis sequi, cupiditate deleniti nihil ipsum debitis blanditiis",
        },
        {
          username: "Jack Nilsson",
          date: "2022/11/1",
          image: " https://miro.medium.com/max/1400/0*0fClPmIScV5pTLoE.jpg",
          message:
            "blanditiis quis sequi, cupiditate deleniti nihil ipsum debitis blanditiis Ducimus tenetur architecto nisi quos animi facere",
        },
        {
          username: "Jenifer Anna",
          date: "2020/11/11",
          image:
            "https://media.istockphoto.com/id/1381221247/photo/beautiful-afro-girl-with-curly-hairstyle.jpg?b=1&s=170667a&w=0&k=20&c=0x91osZOkL8EfhTEEGNa2EeCGN2gdMTNULOsUFW_0i4=",
          message:
            "blanditiis quis sequi, cupiditate deleniti nihil ipsum debitis blanditiis Ducimus tenetur architecto nisi quos animi facereF  blanditiis quis sequi, cupiditate deleniti nihil ipsum debitis blanditiis Ducimus tenetur architecto nisi quos animi facere",
        },
      ],
    };
  },
};
</script>

<style lang="scss" scoped>
@import "@/assets/scss/variables/size.scss";
.image-container {
  img {
    width: 100%;
    height: 350px;
    border-radius: 90px 100px 250px 100px;
  }
}
</style>
